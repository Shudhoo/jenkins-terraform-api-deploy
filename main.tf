...
module "ec2" {
  ...
  subnet_ids = tolist(module.networking.public_subnet_output)[0]
  sg_ids = module.aws_security_group.proj_sg_ec2_output
  enable_public_id = true
  user_data_script = templatefile("./template/ec2_apache_install.sh", {})
...
module "load_balancer_target_group" {
  ...
  vpc_id = module.networking.vpc_output
  ec2_id = module.ec2.ec2_public_id
}
...
module "load_balancer" {
  ...
  is_external = false
  lb_type = "application"
  sg = module.aws_security_group.proj_sg_ec2_output
